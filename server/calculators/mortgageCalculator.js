...
function calculate(inputs) {
  ...
  // --- CA: amortization warning (insured = <20% down, max 25yr) ---
  ...
  // --- US: PMI estimate when LTV >80% ---
  let pmi = null;
  if (marketKey === 'US' && lvr > 80) {
    const pmiAnnualLow = Math.round(loanAmount * 0.005);
    const pmiAnnualHigh = Math.round(loanAmount * 0.015);
    pmi = {
      monthlyLowCents: Math.round(pmiAnnualLow / 12),
      monthlyHighCents: Math.round(pmiAnnualHigh / 12),
      note: 'PMI can typically be removed once you reach 20% equity.',
    };
  }

  // --- NZ: Low equity premium note when LVR >80% ---
  let nzLepNote = null;
  if (marketKey === 'NZ' && lvr > 80) {
    nzLepNote = mktConfig.lvrThresholdDetail ||
      'NZ banks charge a low equity premium (LEP) — typically 0.25%–1.0% added to interest rate.';
  }
  ...
  if (frequency === 'weekly') {
    ...
    // Accelerated bi-weekly: half the monthly payment (pays off loan faster)
  ...
  if (repaymentType === 'IO') {
    ...
    return buildResult({
      repaymentAmount, totalInterest, totalRepaid, lvr, lvrWarning, loanAmount,
      chartData, yearlyBreakdown, marketKey, terminology, mktConfig,
      cmhcPremium, caAmortizationNote, pmi, nzLepNote,
    });
  }
  ...
  return buildResult({
    repaymentAmount, totalInterest: totalInterestPaid, totalRepaid, lvr, lvrWarning, loanAmount,
    chartData, yearlyBreakdown, marketKey, terminology, mktConfig,
    cmhcPremium, caAmortizationNote, pmi, nzLepNote,
  });
}

function buildResult({ repaymentAmount, totalInterest, totalRepaid, lvr, lvrWarning, loanAmount,
  chartData, yearlyBreakdown, marketKey, terminology, mktConfig,
  cmhcPremium, caAmortizationNote, pmi, nzLepNote }) {
  return {
    repaymentAmount,
    ...
    lvrThresholdNote: lvrWarning ? (mktConfig.lvrThresholdNote || null) : null,
    cmhcPremium,
    caAmortizationNote,
    pmi,
    nzLepNote,
