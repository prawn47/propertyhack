/**
 * newsletter-generate queue — dual-mode for CF Workers and local dev
...
let newsletterGenerateQueue;

if (isCFWorkers) {
  // CF Workers — use CF Queues adapter
  const { CFQueue } = require('./cfAdapter');
  newsletterGenerateQueue = new CFQueue('newsletter-generate');
} else {
  // Local / traditional server — use BullMQ + Redis
  const { Queue } = require('bullmq');
  newsletterGenerateQueue = new Queue('newsletter-generate', {
    connection,
    defaultJobOptions: {
      attempts: 3,
