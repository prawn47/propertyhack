/**
 * article-process queue — dual-mode for CF Workers and local dev
...
let articleProcessQueue;

if (isCFWorkers) {
  // CF Workers — use CF Queues adapter
  const { CFQueue } = require('./cfAdapter');
  articleProcessQueue = new CFQueue('article-process');
} else {
  // Local / traditional server — use BullMQ + Redis
  const { Queue } = require('bullmq');
  articleProcessQueue = new Queue('article-process', {
    connection,
    defaultJobOptions: {
      attempts: 3,
...
module.exports = { articleProcessQueue };
