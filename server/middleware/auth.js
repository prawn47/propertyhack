...
const authenticateRefreshToken = async (req, res, next) => {
  try {
    ...
    if (error.name === 'JsonWebTokenError') return res.status(401).json({ error: 'Invalid refresh token' });
    if (error.name === 'TokenExpiredError') return res.status(401).json({ error: 'Refresh token expired' });
