...
export function CountryProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const prefCountry = (prefs?.defaultCountry as string | undefined)?.toUpperCase();
    ...
    if (user) {
      try {
        await updateProfile({
          preferences: {
            ...user.preferences,
            ...(({ defaultCountry: upper } as unknown) as object),
          } as Parameters<typeof updateProfile>[0]['preferences'],
        });
      } catch {
