...
const SaveScenarioButton: React.FC<SaveScenarioButtonProps> = ({
  ...
  if (!isAuthenticated) {
    return (
      <p className="text-sm text-content-secondary">
        <Link
          to="/login"
          className="text-brand-gold underline underline-offset-2 hover:text-brand-primary transition-colors"
        >
          Sign in
        </Link>{' '}
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to save scenario. Please try again.';
      const isLimit = msg.toLowerCase().includes('100') || msg.toLowerCase().includes('limit');
      setErrorMessage(
        isLimit
          ? 'You have reached the maximum of 100 saved scenarios. Please delete some before saving more.'
          : msg
      );
      setModalState('error');
    }
    ...
    <>
      ...
      {modalState !== 'idle' && (
        ...
        >
          ...
          <div className="relative bg-base-100 rounded-xl shadow-xl max-w-sm w-full p-6 flex flex-col gap-4">
